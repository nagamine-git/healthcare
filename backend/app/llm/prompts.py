...
- **再生成の継続性 (重要)**: ``previous_advice_today`` は本日の前回助言のアクション一覧。
  そこに training/cardio があり、(a) 完了済み (recent_workouts_14d で確認) でも
  (b) コンディション悪化でも (c) 実行可能時間切れでもないなら、**再生成で黙って消さない**。
  維持するか時刻を調整して残す。意図的に外す場合は rationale に理由を 1 文書く。
  **ただし継続性より 48h ルール・モダリティ選択が優先**: 前回助言の運動が 48h ルールに
  違反している (例: 昨夜 21時に全身筋トレ実施済みなのに今日も全身筋トレ) なら、
  そのまま維持せず **正しいモダリティ (cardio 等) に差し替える**。
...
- **筋トレ不足なら積極的に刺激を (under-training 対策・最重要)**: ``strength_days_14`` が 6 未満
  ...
  ``body_parts.groups`` で recovery_pct が低い部位 (例: 昨日走った翌日の脚) は外す。
  朝 BB が低め (30-50) でも「回復済み部位への自重刺激は回復を妨げない」ので提案してよい
  (全面休養ではなく "脚以外の自重筋トレ" に振る)。就寝 3 時間前以降なら短時間・低強度で。
- **必要なアクションだけを提案** (1 個でも良い)。穴埋めで増やさない
- **週次フレームワークに従う (最優先・単調さ回避)**: ``training_framework`` が今日のモダリティを
  与える。**これを主権として従うこと**。
  - ``modality == "strength"`` → その日は筋トレ。``split.main_lifts`` の **2 種目を主種目に必ず使い**
    (押す/引く/脚のどれか)、``split.accessory`` と ``split.core`` を 1 つずつ加える (計 3-4 種目)。
    主種目は固定して ``load_suggestions`` で漸進、補助は日替わりでローテ。これで「毎回同じ 3 種目」を防ぐ。
  - ``modality`` が ``"hiit"|"kata"|"z2"`` → その日は有酸素。``detail`` の内容を屋内・準備ゼロで確定
    (kata=木刀素振り連続 / hiit=タバタ・ダンベルコンプレックス / z2=ラッキング等)。筋トレ主種目は入れない。
  - ``weekly`` は週の消化状況 (筋トレ/有酸素の 7 日実績と目標)。回復が極端に悪い日は強度だけ落とす。
  - **例外**: ``training_framework`` が null / コンディション極端悪化のみ、以下の従来ロジックにフォールバック。
- **モダリティ選択 (フォールバック)**: ``recent_workouts_14d`` の **直近 7 日** を以下のモダリティに分類してカウント:\n  - 筋トレ系 (type に strength / weight 等)\n  - 中強度有酸素 (walking / hiking / rucking / jog / cycling / boxing / cardio / VR boxing)\n  - 高強度有酸素 / HIIT (avg_hr 高、interval 系)\n  - その他 (ストレッチ・ヨガ等)\n  本日のモダリティは **直近 7 日で最も不足しているもの** を最優先で選ぶ。例: 筋トレ 4 / 中強度有酸素 0 / HIIT 0 → 今日は中強度有酸素を提案。直近 48h 以内に同部位 (push/pull/legs) の筋トレが入っているなら本日は別部位 or cardio に振る。**全身 (full body) 筋トレは全部位に該当する — 直近 48h 以内に全身筋トレの実績があれば、本日の筋トレ提案は不可。運動枠は cardio に振る**。\n- **同種目連続を避ける**: 同じ種目 (例: ベンチプレス) を 48h 以内に繰り返さない。回復を意識する
- **部位整合 (body_parts と必ず連動)**: 筋トレを出す日は ``body_parts.today_should_train`` (本日の推奨部位) を優先的にメニューへ反映し、``body_parts.groups`` で ``recovery_pct`` が低い部位 (直近に負荷=回復中) を当てる種目は外す。全身メニューでも各種目が当てる部位を推奨部位へ寄せる。これにより画面の「部位別ステータス」カードと処方が一致する (利用者はこの不一致を嫌う)。例: today_should_train=[引く, 脚] なら、ローイング系と脚種目を必ず含め、recovery_pct の低い押す/肩は軽めか除外
...
- **有酸素 (シャドーボクシング / 木刀素振り連続 / 自重・ダンベルサーキット / 加重足踏み。屋内・準備ゼロ種目で確定提案する —
  ...
  - **ミリタリー自重サーキット処方ガイド**: 5-8 種目を「40秒運動+20秒休憩」等で 2-4 周。
    ...
    プランク / デッドバグ から。**シットアップ・フラッターキックは腰既往、ジャンプ系・
    バーピーは騒音のため出さない**
