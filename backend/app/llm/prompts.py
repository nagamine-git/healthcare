...
- body_battery: **「鍛えるか休むか」の判断は朝の値 (morning) を基準にする** — 現在値 (current) は
...
- **モダリティはひとつに確定する**: action の title に「ジョグ or ラッキング」のように **複数候補を or で並べない**。最終的に 1 つに絞り、その種目で exercises を埋める。曖昧な「軽トレーニング」「軽い有酸素」のみの title も禁止。`name` まで具体に決める (例: 「シャドーボクシング (心拍 130) 30分」「加重足踏み (リュック 5kg、心拍 125) 30分」「木刀素振り連続 (心拍 135) 20分」)
...
- **メニュー構築原則**:
  ...
  - VR boxing は装着が面倒なため**基本提案しない** (本人が実施した履歴の解釈には使ってよい)
