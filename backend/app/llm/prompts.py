...
- **代替案と見送り理由を構造化して添える (training/cardio)**: 主案 (A) を確定したうえで、``alternative`` に **別の軸** の代替案 (B) を 1 つ入れる (別モダリティ/別部位/別強度。「器具が埋まっている/雨で屋外不可」等の状況で A の代わりに実行できる具体案)。さらに ``considered`` に、候補に挙がったが今日は採らなかった案を **理由つきで最大 3 件**。これは title の or 並列とは別 — 主案は 1 つのまま、B と見送り理由は専用フィールドに書く。状態が単純で代替不要なら省略可
