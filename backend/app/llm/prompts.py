...
両手なら ``8kg×2``、片手なら ``12kg`` のように表記。リュックサックでの加重 (加重踏み台昇降 等) は
...
- **モダリティはひとつに確定する**: action の title に「ジョグ or ラッキング」のように **複数候補を or で並べない**。最終的に 1 つに絞り、その種目で exercises を埋める。曖昧な「軽トレーニング」「軽い有酸素」のみの title も禁止。`name` まで具体に決める (例: 「踏み台昇降 (リュック 5kg、心拍 140)」「VR boxing (心拍 145)」「シャドーボクシング (心拍 130)」)
...
- 判断指針:
  ...
  - 歩数 5000 未満 → 夜に短時間の中強度有酸素を提案 (心拍 140 で 20 分の踏み台昇降・VR boxing)
...
- **有酸素 (踏み台昇降 / VR boxing / シャドーボクシング 等。屋内種目で確定提案する —
  屋外に出られない前提なので、ランニング / ジョグ / 屋外ウォーキングを主提案にしない)**:
  - **ラッキング置き換え案**: 踏み台昇降を処方するときは、exercises の notes に
    「外に出られる場合は同条件のラッキングに置き換え可 (リュック同重量・同心拍・同時間)」を
    添えてよい。**title や name は踏み台昇降のまま 1 つに確定** (「A or B」表記は引き続き禁止)
  ...
  - **リュック加重 (加重踏み台昇降 等)**: リュック重量は **必ず ``recent_training_prescriptions_21d`` の同種目処方履歴を参照して決定**。実績無しの場合のみ 3kg スタート、それ以外は直近 2-3 回の処方平均を基準に達成 RIR を見て ±2kg 程度で漸進。例: 直近処方が 8kg/5kg/4kg なら次回は 5-8kg のレンジから選ぶ。**勝手に 3kg まで下げてはならない**。腰の既往から上限 12kg を超えない (過去のラッキング処方履歴も同じ加重感覚の参考にしてよい)
  - **踏み台昇降はフラットベンチを使用**。リード脚は左右交互、心拍ターゲットでテンポ調整
                ...
                "items": {
                    ...
                    "properties": {
                        ...
                        "title": {
                            "type": "string",
                            "description": (
                                "アクション名。短く。例: 踏み台昇降 (心拍 140) / ダンベルスクワット 12kg×2 / 軽食。"
                        ...
                        "exercises": {
                            ...
                            "description": (
                                ...
                                "name は素直に: '踏み台昇降', 'VR boxing', 'シャドーボクシング', '木刀素振り: 大素振り' 等。"
