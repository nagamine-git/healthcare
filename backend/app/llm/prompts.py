...
- **筋トレの種目は ``training_framework`` が唯一の決定者 (最優先・単調さ回避)**: これが今日の
  ...
  - ``modality`` が ``hiit|swing|z2``: ``detail`` を屋内・準備ゼロで確定。
    **swing = 室内リズム有酸素の枠**で、シャドーボクシング / 木刀素振り (蹲踞・股割りの切り返し) /
    ペットボトル跳躍素振り(すり足) は**どれも同じ枠**。detail の候補から 1 つ選んで確定し、
    直近と同じものが続かないよう変える (刺激は同じなので入れ替えは継続率のため)。
    hiit=タバタ形式の王道動作 (ダンベルプッシュプレス/ダンベルクリーン/自重スクワット/マウンテンクライマー) /
    z2=ジョグ・足踏み等。筋トレ主種目は入れない。
