...
class Settings(BaseSettings):
    ...
    user_equipment: list[str] = Field(
        default_factory=lambda: [
            "トレーニングマット",
            "フラットベンチ",
            "ダンベル (固定式。2 / 4 / 8 / 12 / 16 / 20 kg のみ。10kg や 6kg などの中間サイズは無い。"
            "8→12 のような飛びは避けられない)",
            "プレート (単体で握って使える。1kg×4 + 2kg×16 = 計 36kg。ゴブレットスクワット・"
            "フロントレイズ・加重コア等の \"プレート保持\" 種目では 1kg 刻みで細かく調整可。"
            "ただしダンベルには装着できない=ダンベルの重量は固定式のまま)",
            "素振り用木刀",
            "0.5L ペットボトル (半分だけ水を入れて ≈250g に加重。2L 満水 (2kg) は肩・肘・手首を痛めやすく"
            "振りが遅くなり運動の速度特異性に反する=軽量の 0.5L 半量に変更。素振りの手の内 "
            "(グリップ・操作) 強化、軽い加重キャリー/レイズ、跳躍素振りの加重に使える)",
            "【無いもの】アブローラー・プッシュアップバー・懸垂バー・トレーニングマシン・"
            "踏み台・リュック(加重)・VR は持っていない。これらを使う種目 (懸垂/ラットプル/"
            "アブローラー/ラッキング/加重足踏み/踏み台昇降/プッシュアップバー使用) は提案しない",
        ]
    )
    # 候補種目 (LLM はこの中から組む)
    user_training_options: list[str] = Field(
        default_factory=lambda: [
            ...
            "その場足踏み/室内ウォーキング (自重・回復日や ながら 運動向き)",
            "ダンベルプッシュプレス (下半身の反動で頭上へ押し上げる王道。心拍 145-155)",
            "ダンベルクリーン (床/腿から肩へ引き上げる王道の全身動作。腰は反らせない)",
            "マウンテンクライマー (スロー・低騒音。自重の全身有酸素)",
            "ファーマーズマーチ (12-16kg×2 を両手に提げてその場行進。握力・体幹・姿勢 + "
            "中強度有酸素。脊柱ニュートラル維持、すくめず肩を下げる)",
            "低騒音 HIIT (Tabata / EMOM、週 1-2 回上限。シャドーボクシング全力 / "
            "ダンベルプッシュプレス / ダンベルクリーン等。ジャンプ系は階下騒音のため不可)",
            "静音バーピー (ジャンプ無し・ステップバック式。ダンベル保持可。全身高心拍の HIIT。"
            "着地は静かに、飛び跳ねない)",
            "ペットボトル跳躍素振り (0.5L ペットボトル半量を両手に持ち、すり足 (足を床から離さず"
            "体重移動のみ) で振り込む。真のジャンプ・着地衝撃は無いため腰既往・階下騒音への"
            "影響は最小。運動量確保が目的。100本×2-3セット)",
            "室内ウォーキング/家事活動 (回復日の NEAT 確保)",
            # 木刀素振り (室内・剣道系。蹲踞 or 股割りの低姿勢で行う)
            "木刀素振り (蹲踞): 大素振り",
            "木刀素振り (蹲踞): 正面素振り",
            "木刀素振り (蹲踞): 左右面素振り",
            "木刀素振り (蹲踞): 左右小手素振り",
            "木刀素振り (蹲踞): 左右胴素振り",
            "木刀素振り (蹲踞): こてこてめんめんどうどう (連続切り返し)",
            "木刀素振り (蹲踞): 高速切り返し",
            "木刀素振り (股割り): 正面素振り",
            "木刀素振り (股割り): 左右面素振り",
            "木刀素振り (股割り): 小手素振り",
            "木刀素振り (股割り): 胴素振り",
            "木刀素振り (股割り): こてこてめんめんどうどう (連続切り返し)",
            "木刀素振り (股割り): 高速切り返し",
            # ダンベル筋トレ
            "ダンベルベンチプレス",
            "ダンベルショルダープレス",
            "ダンベルロー (片手)",
            "ダンベルゴブレットスクワット",
            "ダンベルスクワット (2本サイド持ち。脊柱中立を保てる深さまで・ヒンジ化しないフォーム最優先)",
            "ダンベルルーマニアンデッドリフト (12kg 上限)",
            "ダンベルヒップスラスト",
            "ダンベルカール",
            "ダンベルランジ",
            # 下半身の主軸。固定ダンベルでは両脚種目が「脚より先に握力・上背部が限界」に
            # なるため、片脚化して実効負荷を倍にするのが漸進の主手段になる。
            "ブルガリアンスクワット (ベンチ・片脚。下半身の主軸)",
            "ダンベルステップアップ (ベンチ・片脚)",
            # 片脚ヒンジ。両脚で高重量を持つより脊柱への剪断力が小さく、腰既往に対して安全側。
            "片脚ルーマニアンデッドリフト (バランス+ハム。軽い重量で効く)",
            # 自重
            "プッシュアップ (床。ワイド/ダイヤモンド/デクライン(足をベンチ)/パイク)",
            "カーフレイズ (自重 / ダンベル加重)",
            "プランク",
            # モビリティ / 回復
            "動的ストレッチ (股関節 / 胸郭 / 肩)",
    ...
    # 法人版の衝動買い閾値 (円)。freee の費目内訳/会計期間から「1日あたり裁量経費」を
    # 実データで計算できない時のフォールバック既定値。ルールは個人と同じ 24h 保留。
    corporate_impulse_hold_jpy: int = 10000
    ...
    # PC 仕事を切り上げる、就寝(寝つく)からの逆算。2つの機序が重なる (clinical):
    #  1. 画面は至近距離の強い光源で、夜のメラトニン分泌を抑制し入眠位相を後退させる
    #     (照明を落とす時刻と同じ根拠・同じラグ)
    #  2. 仕事による**認知的覚醒**。就寝前の認知的覚醒 (pre-sleep cognitive arousal) と
    #     仕事の反芻は入眠潜時延長・睡眠の質低下の強い予測因子で、光とは独立に効く。
    # 光だけなら画面を暗くすれば済むが、認知的覚醒は「やめる」以外に手が無いので、
    # ここは「照明↓」とは別の**行動の締切**として分けて出す。
    work_to_bed_lead_min: int = 90
    # 入眠潜時 (布団に入ってから寝つくまで) の既定値。健常成人の正常範囲は 10-20 分
    # (AASM。30 分超が続くと不眠の閾値) の中央として 15 分を採る。
    # ⚠️ **clinical な既定値であって本人の実測ではない**。Garmin は寝ついた時刻しか
    # 測らず、布団に入った時刻は観測できない (体動から推定する検出器を作って検証したが
    # 「就寝が遅い夜ほど睡眠圧が高く入眠は速い」という確立した関係を 24 通りの
    # パラメータどれでも再現できなかった: r=-0.09〜-0.24, いずれも p>0.05)。
    # 本人が「布団に入った」を記録した夜が `sleep_onset_min_nights` 夜たまれば、
    # sleep_plan が自動で本人の実測 median に切り替える。
    sleep_onset_latency_min: int = 15
    # 実測 median へ切り替えるのに必要な記録夜数。median が外れ値1つで動かない最小限。
    sleep_onset_min_nights: int = 5
    ...
    # 起きてから家を出るまでの身支度時間 (分)。**personal**: 生活様式で大きく変わる
    # (育児の有無・朝食を家で摂るか等)。予定からの逆算の既定値で、都度上書きもできる。
    prep_to_departure_min: int = 45
    ...
    user_starting_weights: dict[str, str] = Field(
        default_factory=lambda: {
            ...
            "ダンベルカール": "4kg×2",
            "ダンベルランジ": "4kg×2",
            # 片脚種目は両脚より実効負荷が倍になるので、同じ刺激をより軽い重量で得られる
            "ブルガリアンスクワット": "8kg×2",
            "ダンベルステップアップ": "8kg×2",
            "片脚ルーマニアンデッドリフト": "8kg",
    ...
    garden_catalog: list[dict] = Field(
        default_factory=lambda: [
            ...
            {"kind": "cultural_input", "source": "manual",
             "dimensions": ["growth_mindset", "universalism"],
             "base": 1.1, "evidence": "ラジオ/Podcast/ニュースの継続インプット→知識・視点・言語"},
            {"kind": "teaching", "source": "manual",
             "dimensions": ["benevolence", "growth_mindset"],
             "base": 1.1, "evidence": "教える/共有→protégé effect(学びが定着)・向社会性"},
            {"kind": "planning", "source": "manual",
             "dimensions": ["self_direction", "ownership", "internal_locus"],
             "base": 1.2, "evidence": "計画・週次レビュー→主体性・実行率 (GTD/weekly review)"},
            # --- 身体(脳と気力の土台)---
            ...
            {"kind": "mobility", "source": "manual",
             "dimensions": ["internal_locus", "ownership"],
             "base": 0.9, "evidence": "ストレッチ/可動性→姿勢・疼痛改善・身体基盤の維持"},
            ...
            {"kind": "music", "source": "manual",
             "dimensions": ["internal_locus"],
             "base": 0.8, "evidence": "音楽聴取(クラシック等)→ストレス低減・気分/集中の調整 (Thoma 2013)"},
            {"kind": "breathwork", "source": "manual",
             "dimensions": ["internal_locus"],
             "base": 0.9, "evidence": "ゆっくり呼吸→副交感優位・HRV上昇・覚醒調整"},
            {"kind": "digital_detox", "source": "manual",
             "dimensions": ["internal_locus", "self_direction"],
             "base": 0.9, "evidence": "刺激断ち/通知オフ→注意回復・衝動制御・自己方向づけ"},
            {"kind": "tidying", "source": "manual",
             "dimensions": ["internal_locus", "ownership"],
             "base": 0.9, "evidence": "環境整備/片付け→認知負荷の低減・集中・主体性"},
            # --- 人・生活(主体性/関係/基盤)---
