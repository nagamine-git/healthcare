...
class Settings(BaseSettings):
    ...
    user_equipment: list[str] = Field(
        default_factory=lambda: [
            ...
            "フラットベンチ",
            "プッシュアップバー (小型・2 つ)",
            "リュックサック (中身の重さで負荷調整可、加重踏み台昇降・加重歩行用)",
            "トレーニングマット",
            "木刀 (素振り用)",
    ...
    user_training_options: list[str] = Field(
        default_factory=lambda: [
            # 有酸素 / コンディショニング (屋内が基本。主提案は必ず屋内種目で確定し、
            # ラッキングは「外に出られる場合の置き換え案」として notes に添えるのみ)
            "ベンチ踏み台昇降 (フラットベンチ、有酸素ベース。リュック加重で強度調整可、静音)",
            "ラッキング (リュック歩行。屋外に出られる日のみ — 主提案にせず、"
            "踏み台昇降の notes に同負荷の置き換え案として添える)",
            "VR boxing (Quest 等、有酸素 + 全身協調。ラウンド間休憩を詰めれば中-高強度も可)",
            "シャドーボクシング (素手 or 2kg ダンベル、有酸素 + 肩持久)",
    ...
    user_weekly_target_hint: str = (
        ...
        "- Z2 有酸素: 3 セッション/週 (踏み台昇降 / VR boxing / シャドーボクシング / 素振り連続。"
        "屋内種目で確定提案。外に出られる日はラッキングへの置き換え可を notes に添える。"
        "頻度 > 時間。1 回 30-60 分で OK)\n"
        "- HIIT または ロング Z2: 1 セッション/週 (低騒音 Tabata 1 本 or 60-90 分の"
        "加重踏み台昇降・VR boxing ロング)\n"
