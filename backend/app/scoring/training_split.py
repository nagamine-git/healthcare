...
CARDIO_ROTATION = ["swing", "hiit", "z2"]
# ``swing`` は「室内・静音・準備ゼロで全身をリズミカルに動かす Z2」の枠。
# シャドーボクシング / 木刀素振り (蹲踞・股割りの切り返し) / ペットボトル跳躍素振り(すり足) は
# **どれも同じ性質**なので 1 つの枠にまとめ、日替わりで好きなものを選べるようにする
# (以前はシャドーだけ z2 側、木刀だけ kata 側に分かれ、跳躍素振りはどこにも無かった)。
# 刺激としては同カテゴリなので、飽きたら差し替える = 継続率のための多様性であって
# トレーニング内容の変更ではない。
_SWING_OPTIONS = (
    "シャドーボクシング (3分+1分休のラウンド制、ジャブ/ストレート/フック + 前後ステップ)",
    "木刀素振り 蹲踞 (こてこてめんめんどうどう / 高速切り返し)",
    "木刀素振り 股割り (こてこてめんめんどうどう / 高速切り返し)",
    "ペットボトル跳躍素振り (0.5L半量・すり足。足を床から離さないので静音)",
)
CARDIO_DETAIL = {
    ...
    "swing": (
        "室内リズム有酸素 (心拍 125-135、Z2)。次のどれでも同じ枠 — その日やりたいものを選ぶ: "
        + " / ".join(_SWING_OPTIONS)
    ),
    "z2": "Z2 有酸素: ジョグ (屋外) or 室内その場足踏み・加重足踏み (心拍 125)",
}
CARDIO_LABEL = {"hiit": "HIIT", "swing": "室内リズム有酸素 (シャドー/素振り)", "z2": "Z2 有酸素"}
