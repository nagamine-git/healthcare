      ...
      >
        <div className="flex items-baseline gap-3">
          ...
          {!open && focus.peak_windows.length > 0 && (
            <span className="text-[10px] tabular-nums text-emerald-300/70">
            ...
            <div className="mt-3">
              ...
              <ul className="flex flex-wrap gap-2">
                {focus.peak_windows.map((w) => (
                  <li
                    key={`${w.start}-${w.end}`}
                    className="rounded-lg border border-emerald-500/40 bg-emerald-500/10 px-2 py-1 font-mono text-xs tabular-nums text-emerald-200"
                  >
                    {w.start}–{w.end}
...
function Component({ label, value }: { label: string; value: number | null }) {
  return (
    <div className="rounded-xl border border-slate-800 bg-slate-900/40 px-2 py-2">
