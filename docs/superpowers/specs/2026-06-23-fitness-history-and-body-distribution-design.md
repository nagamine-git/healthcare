...
## 追補 (2026-06-23): 心肺フィットネス (VO2max) を分布に追加

### 背景・判断
追加運動ベンチマークの是非を「前向き研究で予後と相関するか」で triage した結果:
- **心肺 (CRF/VO2max)**: 全候補で最強の死亡予測因子 (低CRFは喫煙/肥満/高血圧より強い)。
  → 追加価値あり。ただし (1) Garmin が VO2max を既に実測、(2) 最大努力ランニングは片頭痛の
  労作性誘因。よって**新規の最大走テストは作らず、既存 Garmin VO2max を母集団 percentile で
  可視化**する (最大努力ゼロ・データは手元にある)。
- **プランク / ダンベル系 (フロアプレス・ロー)**: 母集団基準はあるが**前向きな死亡/疾患相関
  エビデンスが無い** (筋力の予後価値は握力+腕立てで充足)。→ 医学ベンチマーク基準を満たさず**見送り**。

### 実装 (機能Bの拡張)
- `population_norms.py`: `NORMS["vo2max"]` (性別×年代帯, ml/kg/min, ACSM/FRIEND の目安) を追加。
  `build_distribution(..., vo2max=None)` を拡張し、4つ目の指標
  `{key:"vo2max", label:"心肺フィットネス (VO2max)", unit:"ml/kg/min", source:"ACSM/FRIENDの目安", target:None}`
  を返す。VO2max は高いほど良いが、percentile (下位からの位置) はそのまま正しく機能する。
- `body_distribution.py`: 最新の非null `DailySummary.vo2max` (date 降順) を取得して `build_distribution` に渡す。
- フロント: `DistributionPanel` は指標を汎用描画するため改修不要 (説明文に「心肺」を一言追記のみ)。
- テスト: `build_distribution` が vo2max 指標を含む / vo2max 欠損時 value=None。
